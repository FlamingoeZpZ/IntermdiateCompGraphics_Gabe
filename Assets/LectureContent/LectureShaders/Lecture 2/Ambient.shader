    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            vertexOutput vert(vertexInput v)
            {
                ...
                float3 diffuseReflection =  _attention * _lightColor0.xyz * max(0.f, dot(normalDirection, lightDirection)); // Lambert specific vs ADS
                float3 lightModel = diffuseReflection + UNITY_LIGHTMODEL_AMBIENT.xyz;
                
                return vertexOutput(UnityObjectToClipPos(v.vertex), float4(lightModel * _Color.rgb,1) );
